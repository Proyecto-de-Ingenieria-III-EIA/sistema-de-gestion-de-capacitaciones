...
import { GET_ASSESSMENT_PROGRESS_BY_TRAINING, GET_PROGRESS_IN_TRAININGS, GET_USER_PROFILE } from "@/graphql/frontend/users";
...
export default function UserDetails() {
  ...
  const { data: progressData, loading: progressLoading, error: progressError } = useQuery(GET_PROGRESS_IN_TRAININGS, {
    variables: { userId: id },
    context: {
      headers: {
        "session-token": session?.sessionToken,
  ...
  if (progressError) return <p>Error loading training progress: {progressError.message}</p>;
      ...
      <div className="max-w-4xl mx-auto mt-8 p-4">
        ...
        {trainingProgress.length === 0 ? (
            ...
            trainingProgress.map((training: any) => (
              <div key={training.trainingId} className="border p-4 rounded shadow-sm">
                <h3 className="text-lg font-semibold">{training.trainingTitle}</h3>
